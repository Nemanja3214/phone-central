class Graph:
    ...
    class Vertex:
        ...
        __slots__ = '_element', '_popularity'
    ...
    class Edge:
        ...
        def elements(self):
            """ Vraća elemente vezane za ivicu"""
    ...
    def insert_edge(self, u, v, x):
        ...
        """
        if self.get_edge(u, v) is not None:    # ukoliko postoji ivica samo treba da doda novi element u listu elemenata
            self.get_edge(u, v).insert_element(x)
            return
        e = self.Edge(u, v)  # ukoliko ne postoji ivica, kreira se i doda novi element u listu elemenata
        self._outgoing[u][v] = e
        self._incoming[v][u] = e
        e.insert_element(x)
        return e

    def find_all_elements_surrounding_edges(self, v):
        result = []
        for edge in self.incident_edges(v):
            result.extend(edge.elements())
        for edge in self.incident_edges(v, False):
            result.extend(edge.elements())
        return result

    def find_all_elements_incoming_edges(self, v):
        result = []
        for edge in self.incident_edges(v, False):
            result.extend(edge.elements())
        return result
